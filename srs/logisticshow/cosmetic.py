...
def talk(txt: str = None) -> None:
    txt = txt if txt is not None \
        else random.choice(['Работай нахуй!', 'Вот хуй!', 'Ща порву тебя нахуй!', 'Йобушки воробушки', 'Пиздец',
                            'Ебааать', 'Йоб меня в сраку', 'Ебись вертись', 'Мммм хуита', 'Ля ля бля', 'Нихуясе',
                            'Ебать мой хуй', 'Хуё моё', 'Ебучий случай', 'Писос', 'Йоб проёб', 'Ниибёт', 'Не жопься',
                            'Дятел блин', 'Сверхебически', 'Мать перемать', 'Не косоёбся', 'Етись крутись'])
    talk_aloud(txt)

def talk_and_print(txt: str, color: str = "r"):
    my_print(txt=txt, color=color)
    talk(txt)
